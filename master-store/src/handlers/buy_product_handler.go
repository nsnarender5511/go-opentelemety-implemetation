...
func (h *MasterStoreHandler) UpdateStoreStock(c *fiber.Ctx) (err error) {
	ctx := c.UserContext()
	h.logger.InfoContext(ctx, "Master Store: Processing purchase request")

	var req apirequests.ProductBuyRequest
	if parseErr := c.BodyParser(&req); parseErr != nil {
		h.logger.ErrorContext(ctx, "Master Store: Invalid purchase request format", slog.String("error", parseErr.Error()))
	...
	if validatorErr := validator.ValidateRequest(&req); validatorErr != nil {
		h.logger.WarnContext(ctx, "Master Store: Invalid purchase request data", slog.String("validator_error", validatorErr.Message))
		err = validatorErr
		return
	}
	...
	h.logger.DebugContext(ctx, "Master Store: Processing purchase for product", slog.String("product_name", productName), slog.Int("quantity", quantity))
	...
	h.logger.InfoContext(ctx, "Master Store: Coordinating purchase with inventory system", slog.String("product_name", productName), slog.Int("quantity", quantity))
	...
	h.logger.InfoContext(ctx, "Master Store: Purchase transaction completed",
