...
**Related Pages:** ./Telemetry_Setup.md, [`common/log/log.go`](../../common/log/log.go), ../development/Configuration_Management.md
...
*   **OTel Integration:** When `ENVIRONMENT=production`, the `otelslog.NewHandler` acts as a bridge.
    ...
    *   These OTel LogRecords are then processed by the configured OTel `LoggerProvider` (setup in ./Telemetry_Setup.md via `common/telemetry/log/exporter.go`).
...
*   **Common Pitfalls / Questions:**
    ...
    *   Why are my logs not appearing in SigNoz? (Check `ENVIRONMENT` variable (and `globals.Init` override), ensure OTLP handler is active in `log.Init`, check ./Telemetry_Setup.md, check Collector pipeline for logs, check SigNoz ingestion key).
